...
-- Tracks cryptographic signatures for approved financial documents.
...
CREATE TABLE IF NOT EXISTS finance_signatures (
    ...
    hash_value TEXT NOT NULL, -- SHA-256 of the JSON payload
    signature TEXT NOT NULL, -- The cryptographic signature
    signed_by UUID REFERENCES auth.users(id),
    signed_at TIMESTAMPTZ DEFAULT now(),
    signature_algorithm TEXT DEFAULT 'RSA-SHA256',
    
    public_key_fingerprint TEXT, -- To identify which key was used
...
CREATE TABLE IF NOT EXISTS ai_finance_flags (
    ...
    flag_type TEXT NOT NULL, -- e.g. 'high_amount', 'duplicate_payee', 'budget_exceeded', 'unusual_time'
...
-- Manages async submission and retries for LHDN e-Invoices.

CREATE TABLE IF NOT EXISTS gov_submission_queue (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    firm_id UUID NOT NULL REFERENCES firms(id),
    
    document_type TEXT NOT NULL, -- 'invoice', 'consolidated', 'cancel'
    document_id UUID NOT NULL, -- Link to e_invoices.id
    
    status TEXT DEFAULT 'queued', -- 'queued', 'processing', 'submitted', 'failed', 'retrying'
...
-- Index for the worker to pick up pending jobs
CREATE INDEX idx_gov_queue_pending ON gov_submission_queue(status, next_retry_at) 
WHERE status IN ('queued', 'retrying');


-- 4️⃣ Trust Account Ledger (Foundation)
-- Legal firms must separate Office Account vs Client Account (Trust).
-- This is a double-entry ledger system for Trust Accounts.

CREATE TABLE IF NOT EXISTS trust_accounts (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    firm_id UUID NOT NULL REFERENCES firms(id),
    
    account_name TEXT NOT NULL, -- e.g. "Public Bank Client Account"
...
CREATE TABLE IF NOT EXISTS trust_ledger_entries (
    ...
    case_id UUID REFERENCES cases(id), -- Trust money MUST be linked to a case (Client)
    client_id UUID, -- Optional direct link if no case
    
    transaction_date DATE NOT NULL,
    description TEXT NOT NULL,
    
    debit NUMERIC(15,2) DEFAULT 0.00, -- Money Out
    credit NUMERIC(15,2) DEFAULT 0.00, -- Money In
    balance_after NUMERIC(15,2) NOT NULL,
    
    transaction_type TEXT, -- 'deposit', 'withdrawal', 'transfer'
    reference_no TEXT, -- Cheque No, TT Ref
...
-- 5️⃣ Additional Permissions for new modules
