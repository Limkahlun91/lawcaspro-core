...
DECLARE
    v_case_id BIGINT; -- Fixed: Explicit scalar type instead of RECORD
    v_invoice_id UUID;
    v_milestone_status TEXT;
BEGIN
    -- Lock milestone row
    SELECT status, case_id INTO v_milestone_status, v_case_id 
    ...
    -- Create Invoice (Assuming client_id is in cases table)
    -- We need to join to get client_id
