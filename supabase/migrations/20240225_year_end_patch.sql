...
BEGIN
    ...
    -- 3. Strict 12-Month Period Validation
    SELECT COUNT(*) INTO v_period_count 
    FROM accounting_periods 
    WHERE firm_id = v_firm_id AND year = p_year AND is_closed = TRUE;

    IF v_period_count != 12 THEN
        RAISE EXCEPTION 'Cannot close year: Exactly 12 closed accounting periods required. Found: %', v_period_count;
    END IF;
    ...
    -- 5. Lock GL Entries for Calculation (Prevent concurrent writes)
    PERFORM 1 FROM gl_entries 
    WHERE firm_id = v_firm_id 
    AND EXTRACT(YEAR FROM date) = p_year
    FOR UPDATE;

    -- 6. Calculate P&L (OFFICE Only)
    ...
    -- 10. Post Net Profit/Loss to Retained Earnings (Always Insert)
