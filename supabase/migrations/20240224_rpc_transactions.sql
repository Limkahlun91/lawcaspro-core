...
-- 1. PV Sequence (Per Firm or Global? Let's assume Global Sequence for now, or per-firm trigger. 
-- For simplicity and robustness, a global sequence per year is safer for uniqueness, but per firm is better for SaaS.
-- Let's stick to a simple global sequence for now to ensure uniqueness.)
